...
const TranscriptParser = {
  ...
  /**
   * Post-process segments with custom settings (offsets, speaker splitting, gaps)
   */
  postProcess(segments, options = {}) {
    let processed = [...segments];

    // 1. Split by speaker if enabled
    if (options.splitSpeakers) {
      processed = this.splitBySpeakers(processed);
    }

    // 2. Adjust time offsets if needed (offset in seconds)
    if (options.timeOffset) {
      const offset = parseFloat(options.timeOffset) || 0;
      processed = processed.map(seg => ({
        ...seg,
        time: seg.time !== null ? Math.max(0, seg.time + offset) : null
      }));
    }

    // 3. Mark transitions by time gaps (silence / pause detection)
    if (options.gapThreshold && options.gapThreshold > 0) {
      const threshold = parseFloat(options.gapThreshold);
      for (let i = 1; i < processed.length; i++) {
        ...
        if (prev.time !== null && curr.time !== null) {
          ...
          if (gap >= threshold) {
            curr.isNewConversationBlock = true;
          }
        }
      }
    }
  ...
  /**
   * Split segment text by speaker labels
   */
  splitBySpeakers(segments) {
    const result = [];
    const speakerColon = /^\s*([A-Za-z0-9\s_-]{1,25}):\s*(.*)$/;
    const speakerBracket = /^\s*[\[\(]([A-Za-z0-9\s_-]{1,25})[\]\)]\s*(.*)$/;

    for (const seg of segments) {
      if (!seg.text) continue;
      
      const lines = seg.text.split('\n');
      let currentSpeaker = seg.speaker || null;
      let currentLines = [];
      ...
      for (const line of lines) {
        ...
        let match = trimmed.match(speakerColon) || trimmed.match(speakerBracket);
        if (match) {
          if (currentLines.length > 0) {
            result.push({
              ...
              isNewConversationBlock: seg.isNewConversationBlock
            });
            currentLines = [];
          }
