...
const VideoProcessor = {
   ...
   */
  async loadVideo(file, taskId = null) {
    if (taskId) {
   ...
   */
  async extractFrames(taskId, interval, quality = 1.0, maxWidth = 9999, onProgress = null) {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      try {
        const url = await this._captureFrame(taskId, time, drawWidth, drawHeight);
   ...
   */
  _captureFrame(taskId, time, width, height) {
    return new Promise((resolve, reject) => {
      const video = this._video;
      const canvas = this._canvas;
      const ctx = this._ctx;
      ...
      const onSeeked = () => {
        ...
        try {
          ...
          canvas.toBlob((blob) => {
            ...
            const url = FrameStore.store(taskId, blob, width, height);
            resolve(url);
          }, 'image/png');
