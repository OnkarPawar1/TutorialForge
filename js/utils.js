...
const Utils = {
  ...
  /**
   * Given lists of video files and subtitle files, matches them based on name.
   * Returns matched, unmatchedVideos, unmatchedSubs grouped as tasks.
   */
  matchFiles(videoFiles, subtitleFiles) {
    const matched = [];
    const unmatchedVideos = [...videoFiles];
    const unmatchedSubs = [...subtitleFiles];
    ...
    // First pass: exact clean-name matching
    for (let i = unmatchedVideos.length - 1; i >= 0; i--) {
      const v = unmatchedVideos[i];
      const vClean = cleanName(v.name);
      
      const sIdx = unmatchedSubs.findIndex(s => cleanName(s.name) === vClean);
      if (sIdx !== -1) {
        matched.push({
          videoFile: v,
          subtitleFile: unmatchedSubs[sIdx]
        });
        unmatchedVideos.splice(i, 1);
        unmatchedSubs.splice(sIdx, 1);
      }
    }

    // Second pass: starts-with or partial matching
    for (let i = unmatchedVideos.length - 1; i >= 0; i--) {
      const v = unmatchedVideos[i];
      const vClean = cleanName(v.name);

      const sIdx = unmatchedSubs.findIndex(s => {
        const sClean = cleanName(s.name);
        return sClean.startsWith(vClean) || vClean.startsWith(sClean);
      });
...
const FrameStore = {
  ...
  _taskUrls: new Map(), // taskId → Set of objectURLs

  /**
   * Store a Blob and return its objectURL
   */
  store(taskId, blob, width, height) {
    const url = URL.createObjectURL(blob);
    this._blobs.set(url, blob);
    this._dims.set(url, { width, height });

    if (taskId) {
      if (!this._taskUrls.has(taskId)) {
        this._taskUrls.set(taskId, new Set());
      }
      this._taskUrls.get(taskId).add(url);
    }
    return url;
  },
  ...
  /**
   * Cleanup specific task's stored blobs
   */
  cleanupTask(taskId) {
    const urls = this._taskUrls.get(taskId);
    if (urls) {
      for (const url of urls) {
        URL.revokeObjectURL(url);
        this._blobs.delete(url);
        this._dims.delete(url);
      }
      this._taskUrls.delete(taskId);
    }
  },
